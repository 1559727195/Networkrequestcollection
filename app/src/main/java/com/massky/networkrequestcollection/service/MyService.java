package com.massky.networkrequestcollection.service;
...
import com.massky.networkrequestcollection.thread.Receive_Thread;
...
import static com.massky.networkrequestcollection.util.AES.Encrypt;
...
public class MyService extends Service {//Service 的生命周期
     ...
     * 连接Tcp
     */
    public void connectTCP(int port,String wangguan_ip, final IConnectTcpback iconnect, ICallback iCallback) {
        initSocket(port,wangguan_ip, iconnect, iCallback);
    }
     ...
     * socket初始化
     */
    private void initSocket(int port,String wangguan_ip, final IConnectTcpback iconnect, ICallback iCallback) {
        connect_ctp--;
        clicksSocket = null;
        try {
//            clicksSocket= new Socket("192.168.169.134", 8080);
            ...
            SocketAddress socAddress = new InetSocketAddress(wangguan_ip, port);//wangguan_ip,32678
            //TcpServer 端口 32678
...
//            clicksSocket.setSoTimeout(5000);
            ...
            Receive_Thread receive_thread = new Receive_Thread(clicksSocket, _instance, iCallback);//创建客户端处理线程对象
